 ...
 */
class IndonesianPreprocess extends InternalModule {

    static final ULocale ID_LOCALE = new ULocale.Builder().setLanguage("id").build();
    String formatRules
    ...
    public IndonesianPreprocess() {
        super("IndonesianPreprocess", MaryDataType.TOKENS, MaryDataType.WORDS, ID_LOCALE.toLocale());
        formatRules = this.getClass().getResource('IPreprocess/formatRules.txt').getText('UTF-8')
        rbnf = new RuleBasedNumberFormat(formatRules, ID_LOCALE)
        cardinalRule = "%spellout-numbering"
        ordinalRule = "%spellout-ordinal"
        yearRule = "%spellout-numbering"
    }
